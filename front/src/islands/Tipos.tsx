...
const Tipos: FunctionComponent<csrProps> = (props) => {
    ...
    const getData = async () => {
        ...
        const res = await fetch(`http://localhost:3000/getPelisTipos/${tipo}/${miAuth}`);
    ...
    return (
        <div>
            ...
            {midata.length == 0 && <div>No hay películas de este tipo, ¿desea <strong><a href="/addPeli">añadir alguna</a> o <a href="/listado">volver a listado?</a></strong></div>}
            <div class="container">
                {midata.map((p) => {
                    return (
                        <div class="peli-sola">
